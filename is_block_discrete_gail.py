...
# Import GAIL policy network
from run_block_discrete_gail import GAILPolicyNetwork

# Parse command line arguments
parser = argparse.ArgumentParser(description='Importance Sampling evaluation for Block Discrete GAIL')
parser.add_argument('--model_path', type=str, required=True,
                   help='Path to trained GAIL model checkpoint')
...
### define the GAIL policy model
# - load the trained GAIL policy model
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(f"Using device: {device}")

# Model parameters
n_actions = 2 * n_bins  # VP1 (2 options: 0,1) x VP2 (n_bins bins)
state_dim = STATE_DIM  # 17 features

# Load GAIL checkpoint
checkpoint = torch.load(model_path, map_location=device)

# Use vp2_bins from checkpoint if available, otherwise use CLI arg
ckpt_vp2_bins = checkpoint.get('vp2_bins', n_bins)
if ckpt_vp2_bins != n_bins:
    print(f"Warning: checkpoint vp2_bins={ckpt_vp2_bins} differs from --vp2_bins={n_bins}, using checkpoint value")
    n_bins = ckpt_vp2_bins
    n_actions = 2 * n_bins

# Initialize and load GAIL policy network
policy_network = GAILPolicyNetwork(state_dim=state_dim, vp2_bins=n_bins).to(device)
policy_network.load_state_dict(checkpoint['policy_state_dict'])
policy_network.eval()

print(f"Loaded GAIL policy from: {model_path}")
print(f"State dimension: {state_dim}")
print(f"Number of actions: {n_actions} (VP1: 2 binary x VP2: {n_bins} bins)")
print("Using argmax of policy probabilities for action selection")
...
# Helper function to select actions using GAIL policy and return DISCRETE indices
def select_action_batch_discrete_gail(states, policy_net, device):
    """
    Select best actions for a batch of states using GAIL policy argmax.
    ...
    with torch.no_grad():
        ...
        probs = policy_net(state_tensor)  # [batch_size, n_actions]
        best_action_indices = probs.argmax(dim=1).cpu().numpy()
...
train_model_actions_discrete = select_action_batch_discrete_gail(train_data['states'], policy_network, device)

print(f"Computing model actions (discrete) for {eval_set_name.lower()} data...")
eval_model_actions_discrete = select_action_batch_discrete_gail(eval_data['states'], policy_network, device)
...
## train a softmax classifier to predict model/clinician actions using states on training data
...
\centering
\caption{Importance Sampling Off-Policy Evaluation Results (Block Discrete GAIL)}
\label{tab:is_ope_results_gail}
\begin{tabular}{lcc}
\hline
...
latex_file = 'latex/is_ope_results_gail.tex'
