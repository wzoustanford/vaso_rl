...
def compute_training_step(
    ...
    for ct in range(seq_len - D):
        ...
        # --- Block 6b-2: Reshape and forward through transformer reward model ---
...
def evaluate_validation(
    ...
    with torch.no_grad():
        ...
        for ct in range(seq_len - D):
            ...
            # Reshape and forward through transformer reward model
            state_action_flat = state_action.permute(0, 2, 1, 3).reshape(
                batch_size * action_size, seq_len, state_size + action_size
            )
            rewards_pred = model(state_action_flat)
...
def load_model(filepath: str, device: torch.device = None) -> tuple:
    ...
    Returns:
        model: Transformer reward model
...
def main():
    """Main entry point for training transformer reward generator."""
    import argparse
    
    parser = argparse.ArgumentParser(description='Train transformer reward generator')
    ...
    print(f"Config: epochs={config.num_epochs}, batch_size={config.batch_size}, "
          f"lr={config.learning_rate}, d_model={config.d_model}, "
          f"nhead={config.nhead}, num_layers={config.num_layers}, d_ff={config.d_ff}, dropout={config.dropout}, "
          f"D={config.D}, gamma={config.gamma}, "
          f"vp1_bins={config.vp1_bins}, vp2_bins={config.vp2_bins}, action_size={config.action_size}")

    # Initialize data pipeline (use manual rewards - we generate our own via transformer)
